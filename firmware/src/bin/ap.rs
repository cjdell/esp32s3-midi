...
use esp_hal::{clock::CpuClock, interrupt::software::SoftwareInterruptControl, ram, rng::Rng, timer::timg::TimerGroup};
...
async fn main(spawner: Spawner) -> ! {
    ...
    let access_point_config = Config::AccessPoint(AccessPointConfig::default().with_ssid("esp-radio"));
    ...
    println!("Connect to the AP `esp-radio` and point your browser to http://{gw_ip_addr_str}:8080/");
    ...
    loop {
        ...
        let r = socket.accept(IpListenEndpoint { addr: None, port: 8080 }).await;
        println!("Connected...");
...
async fn connection(controller: WifiController<'static>) {
    ...
    loop {
        let ev = controller.wait_for_access_point_connected_event_async().await;
        match ev {
            Ok(esp_radio::wifi::AccessPointStationEventInfo::Connected(access_point_station_connected_info)) => {
                println!("Station connected: {:?}", access_point_station_connected_info);
            }
            Ok(esp_radio::wifi::AccessPointStationEventInfo::Disconnected(access_point_station_disconnected_info)) => {
